...
(deftest defmacro-minimal
  (is (=
       (macroexpand-1
        `(defmojo ~'MyFirstMojo {} [] (log/info "Hello World!")))
       '(do
          (clojure.core/deftype MyFirstMojo
            ...
            (execute [this] (clojure.maven.mojo.log/with-log log
                              (clojure.maven.mojo.log/info "Hello World!")))
...
(deftest invalid-mojo-annotation
  (is (thrown-with-msg? Exception #"annotation"
        (macroexpand-1 `(defmojo ~'Test
                            {:goalie "test"}
                            []
                          (log/info "hello world!"))))))
...
(deftest invalid-param-pair
  (is (thrown-with-msg? Exception #"parameter"
        (macroexpand-1 `(defmojo
                            ~'Test
                            {:goal "test"}
                            [~'param-name ~'no-map]
